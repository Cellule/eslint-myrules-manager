...
function loadRules(lintConfig, rulePath, target) {
  ...
  if(target === "global" && lintConfig.mergeGlobal) {
    var mergeTargets = toArray(lintConfig.mergeGlobal);
    mergeTargets.forEach(function(mergeTarget) {
      ...
      var rulesObjTarget = loadRulesObject(lintConfig, rulePath, mergeTarget);
      rulesObj = utils.mergeRules(rulesObj, rulesObjTarget);
    });
